...
beforeAll(async () => {
  ...
  await import('../../../supabase/functions/admin-import-public-objectives/index.ts')
})
...
describe('admin-import-public-objectives', () => {
  ...
  it('returns 400 when a required field is missing', async () => {
    ...
    const res = await handler(makeRequest({ records: [{ name: 'Spend 3 Influence' }] })) // missing stage, condition
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/stage/)
    expect(body.error).toMatch(/Record 1/)
  })

  it('returns 400 when stage is not an integer', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const res = await handler(makeRequest({ records: [{ name: 'Test', stage: 'one', condition: 'Do something' }] }))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/stage/)
  })
  ...
  it('returns 200 with imported count on valid payload', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const records = [
      { name: 'Spend 3 Influence', stage: 1, condition: 'Spend 3 influence' },
      { name: 'Spend 8 Resources', stage: 2, condition: 'Spend 8 resources' },
    ]
    const res = await handler(makeRequest({ records }))
