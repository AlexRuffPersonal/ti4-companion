...
describe('admin-import-technologies', () => {
  ...
  it('returns 400 when a required field is missing', async () => {
    ...
    const res = await handler(makeRequest({ records: [{ name: 'Neural Motivator' }] })) // missing colour
  ...
  it('returns 400 when colour enum value is invalid', async () => {
    requireAdmin.mockResolvedValue('user-id')
    const res = await handler(makeRequest({ records: [{ name: 'Neural Motivator', colour: 'purple' }] }))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/colour/)
  })
  ...
  it('returns 200 with imported count on valid payload', async () => {
    requireAdmin.mockResolvedValue('user-id')
    const records = [
      { name: 'Neural Motivator', colour: 'blue' },
      { name: 'Sarween Tools', colour: 'yellow' },
    ]
    const res = await handler(makeRequest({ records }))
  ...
  it('returns 500 when the database insert fails', async () => {
    ...
    const res = await handler(makeRequest({ records: [{ name: 'Neural Motivator', colour: 'blue' }] }))
