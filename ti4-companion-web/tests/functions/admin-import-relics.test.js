...
const makeRequest = (body) => _makeRequest('admin-import-relics', body)
...
beforeAll(async () => {
  ...
  await import('../../../supabase/functions/admin-import-relics/index.ts')
})
...
describe('admin-import-relics', () => {
  ...
  it('returns 400 when name is not a string', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const res = await handler(makeRequest({ records: [{ name: [] }] }))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/name/)
  })
  ...
  it('returns 200 with imported count on valid payload', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const records = [{ name: 'Shard of the Throne' }, { name: 'Crown of Emphidia' }]
    const res = await handler(makeRequest({ records }))
