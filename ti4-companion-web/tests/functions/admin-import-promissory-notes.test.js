...
function makeRequest(body) {
  return new Request('http://localhost/admin-import-promissory-notes', {
    method: 'POST',
...
beforeAll(async () => {
  ...
  await import('../../../supabase/functions/admin-import-promissory-notes/index.ts')
})
...
describe('admin-import-promissory-notes', () => {
  ...
  it('returns 400 when name is not a string', async () => {
    requireAdmin.mockResolvedValue('user-id')
    const res = await handler(makeRequest({ records: [{ name: 0 }] }))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/name/)
  })
  ...
  it('returns 200 with imported count on valid payload', async () => {
    requireAdmin.mockResolvedValue('user-id')
    const records = [{ name: 'Political Favor' }, { name: 'Trade Convoys' }]
    const res = await handler(makeRequest({ records }))
