...
## Functionality
```pseudocode
// Static map of exploration card name → DSL op array.
// Consumed by game-explore-planet and game-explore-frontier.

EXPLORATION_EFFECTS: Record<string, Op[]> = {
  // Industrial deck
...
Note: `Enigmatic Device` stays in the player's play area (not discarded); its ACTION ability is handled separately via `game-use-relic` using `relicEffects.ts`.

## Tests
No standalone test file — covered through `game-explore-planet` and `game-explore-frontier` tests.
