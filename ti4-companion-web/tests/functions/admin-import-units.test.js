...
const makeRequest = (body) => _makeRequest('admin-import-units', body)
...
describe('admin-import-units', () => {
  ...
  it('returns 400 when name is not a string', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const res = await handler(makeRequest({ records: [{ name: true }] }))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/name/)
  })
  ...
  it('returns 200 with imported count on valid payload', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const records = [{ name: 'War Sun' }, { name: 'Carrier' }]
    const res = await handler(makeRequest({ records }))
