# Phase 39 — Leader Card Ability Enforcement

**Date:** 2026-05-19
**Scope:** Full enforcement of all 72 leader abilities across 24 factions (agents, commanders, heroes).
**Rules basis:** LRR §50–51 (Leader Sheet, Leaders). Agent exhaust/ready cycle §51.3–51.4. Commander unlock §51.5–51.8. Hero unlock and purge §51.9–51.12.

---

## 1. Architecture Overview

The design centres on a single new shared module — `shared-leaderEffects.ts` — holding all 72 leader ability definitions as static TypeScript records, mirroring the `relicEffects.ts` / `techEffects.ts` pattern. Three registries live in this file:
...
- **39a**: Infrastructure + all 24 faction agents
- **39b**: All 24 faction heroes
- **39c**: Commander unlock (fn-game-unlock-commander) + all 24 commander passives hooked into affected Edge Functions
...
**AGENT_ABILITIES** — keyed by faction name, value is `Op[]` or handler key string:
```typescript
...
**HERO_ABILITIES** — keyed by faction name:
```typescript
...
**COMMANDER_PASSIVES** — keyed by faction name:
```typescript
...
**AGENT_REACTIVE_TRIGGERS** — maps faction to the trigger types that can prompt their agent as a reactive window:
```typescript
...
### New DSL ops (Phase 39a/39b additions to `abilityDsl.ts`)
...
**Agent handlers:**
- `ssruu_copies_agents` — Yssaril agent copies all other agents' text. Display-only: show card text of all other agents in the modal; no server enforcement. Exhausts the Yssaril agent normally.

**Hero handlers:**
- `creuss_riftwalker` — Swap positions of 2 systems (each contains a wormhole or player's units; neither can be Creuss home or Wormhole Nexus). Requires selecting 2 system keys; updates `game_tiles` positions.
- `mahact_hero` — Move all units from a chosen system's space area to an adjacent system containing another player's ships. Initiates space combat in destination; neither player can retreat or use movement abilities during this combat.
- `winnu_mathis` — Player selects a strategy card; server applies its primary ability for the Winnu player; chosen other players may perform its secondary.
- `letnev_darktalon` — Set `games.game_round_flags.letnev_no_fleet_limit = true`; cleared at end of game round in `game-advance-phase`.
- `nomad_ahk_syl` — Set `games.game_round_flags.nomad_flagship_ignores_tokens = true`; cleared at end of game round.
- `vuil_raith_hero` — For each other player, roll 1d10 per non-fighter ship in or adjacent to a dimensional tear system; capture unit on 1–3. Multi-player dice roll, returns all results in response.
- `xxcha_xxekir` — Optionally discard 1 law from play; look at top 5 agenda cards; choose 2 to resolve as if 1 vote cast for a chosen outcome; discard rest. No other players may resolve abilities during this action.
- `yssaril_kyver` — Each other player reveals 1 action card; for each, Yssaril player may take that card or force that player to discard 3 random cards from hand. Multi-player interaction.

**Commander handlers:**
- `l1z1x_skip_planetary_shield` — In `game-fire-bombardment`, if the active player is L1Z1X with unlocked commander, skip the Planetary Shield check entirely.
- `xxcha_extra_vote_per_planet` — In `game-cast-votes`, count the number of planets the Xxcha player is exhausting and add that count to their vote total.
- `winnu_combat_bonus` — In `game-roll-combat-dice` and `game-roll-ground-combat-dice`, if system is Mecatol Rex, Winnu home system, or contains a legendary planet, add +2 to each result.
- `hacan_trade_good_votes` — In `game-cast-votes`, allow Hacan player to spend trade goods; each adds 2 to vote total.
- `yin_omar_passive` — In `game-research-technology`, treat one prerequisite colour as satisfied if Yin commander is unlocked. Also in `game-produce-units`, allow 1 additional infantry that does not count against Production limit.
...
fetch game_players for caller → player
...
| Faction | Condition |
|---|---|
| Mahact | COUNT DISTINCT factions of captured tokens in fleet pool ≥ 2 |
| Argent Flight | COUNT units capable of AFB/Space Cannon/Bombardment ≥ 6 |
| Nekro | COUNT player.technologies ≥ 3 |
| Titans | COUNT structures on player's planets ≥ 5 |
| Vuil'raith | COUNT DISTINCT systems with gravity rift tile WHERE player has units ≥ 3 |
| Muaat | EXISTS war_sun in game_player_units for player |
| L1Z1X | COUNT dreadnoughts ≥ 4 |
| Naaz-Rokha | COUNT DISTINCT system_key WHERE player has mech ≥ 3 |
| Sol | SUM resource of controlled planets ≥ 12 |
| Saar | COUNT space_dock structures ≥ 3 |
| Letnev | MAX(COUNT non-fighter ships per system) ≥ 5 |
| Jol-Nar | COUNT player.technologies ≥ 8 |
| Yin | player.commander_flags.used_indoctrination = true |
| Hacan | player.trade_goods ≥ 10 |
| Winnu | controls Mecatol Rex OR player.commander_flags.entered_mecatol_combat = true |
| Nomad | COUNT scored secret objectives ≥ 1 |
| Yssaril | player.action_card_count ≥ 7 |
| Arborec | COUNT infantry + mech on controlled planets ≥ 12 |
| Naalu | COUNT fighters on board ≥ 12 |
| Xxcha | SUM influence of controlled planets ≥ 12 |
| Mentak | COUNT cruisers ≥ 4 |
| Empyrean | all other active players share at least 1 system border with this player |
| Sardakk | COUNT DISTINCT non-home controlled planets ≥ 5 |
| Creuss | COUNT DISTINCT systems with alpha/beta wormhole WHERE player has units ≥ 3 |
...
| `game-fire-bombardment` | `BOMBARDMENT` | L1Z1X 2RAM (skip planetary shield — inline pre-check) |
...
| `game-cast-votes` | `CAST_VOTES` | Xxcha Elder Qanoj (extra votes per exhausted planet), Hacan Gila (TG → votes) |
...
LeaderAbilityModal({ leader, faction, leaderType, gamePlayers, onConfirm, onClose })
  ...
  render:
    ...
    if selectionConfig.needs_target_player → <PlayerPicker players={gamePlayers} />
    if selectionConfig.needs_planet       → <PlanetPicker filter={selectionConfig.planet_filter} />
    if selectionConfig.needs_system       → <SystemPicker filter={selectionConfig.system_filter} count={selectionConfig.count} />
    if selectionConfig.needs_choice       → <ChoicePicker options={selectionConfig.options} />
    if no selections needed               → confirmation prompt only
    [CONFIRM] btn-primary → onConfirm(selections)
    [CANCEL]  btn-ghost   → onClose()
```
...
- `handleUseAbility(leader)` → opens `LeaderAbilityModal`
- `handleConfirm(selections)` → calls `resolveLeaderAbility(abilityDefinitionId, leader.id, selections)`
- `handleCommanderWindow(window)` → dispatches `reactive_agent` / `commander_passive` to banner, `commander_reroll` to reroll modal
- `unlockCommander(leaderId)` → calls new `fn-game-unlock-commander`

---

## 7. Spec File Decomposition

### Phase 39a — Infrastructure + Agents (14 spec files)

| Spec file | Actual file |
|---|---|
| `migration-048-leader-abilities` | `supabase/migrations/048_leader_abilities.sql` |
| `shared-leaderEffects` | `supabase/functions/_shared/leaderEffects.ts` |
| `shared-abilityDsl-p39a` | `supabase/functions/_shared/abilityDsl.ts` |
| `shared-abilityHandlers-p39a` | `supabase/functions/_shared/abilityHandlers.ts` |
| `fn-game-resolve-ability-p39a` | `supabase/functions/game-resolve-ability/index.ts` |
| `fn-game-advance-phase-p39a` | `supabase/functions/game-advance-phase/index.ts` |
| `fn-game-activate-system-p39a` | `supabase/functions/game-activate-system/index.ts` |
| `fn-game-produce-units-p39a` | `supabase/functions/game-produce-units/index.ts` |
| `fn-game-assign-hits-p39a` | `supabase/functions/game-assign-hits/index.ts` |
| `lib-leaderConstants` | `src/lib/leaderConstants.js` |
| `component-LeaderAbilityModal` | `src/components/game/LeaderAbilityModal.jsx` |
| `hook-useLeaders-p39a` | `src/hooks/useLeaders.js` |
| `component-LeaderPanel-p39a` | `src/components/game/LeaderPanel.jsx` |
| `component-GameScreen-p39a` | `src/components/game/GameScreen.jsx` |

### Phase 39b — Heroes (5 spec files)

| Spec file | Actual file |
|---|---|
| `shared-leaderEffects-p39b` | `supabase/functions/_shared/leaderEffects.ts` |
| `shared-abilityHandlers-p39b` | `supabase/functions/_shared/abilityHandlers.ts` |
| `fn-game-resolve-ability-p39b` | `supabase/functions/game-resolve-ability/index.ts` |
| `fn-game-advance-phase-p39b` | `supabase/functions/game-advance-phase/index.ts` |
| `component-LeaderAbilityModal-p39b` | `src/components/game/LeaderAbilityModal.jsx` |

### Phase 39c — Commander Unlock + Passives (22 spec files)
...
| `shared-leaderEffects-p39c` | `supabase/functions/_shared/leaderEffects.ts` |
| `shared-abilityHandlers-p39c` | `supabase/functions/_shared/abilityHandlers.ts` |
| `fn-game-produce-units-p39c` | `supabase/functions/game-produce-units/index.ts` |
| `fn-game-research-technology-p39c` | `supabase/functions/game-research-technology/index.ts` |
| `fn-game-assign-hits-p39c` | `supabase/functions/game-assign-hits/index.ts` |
| `fn-game-commit-ground-forces-p39c` | `supabase/functions/game-commit-ground-forces/index.ts` |
| `fn-game-roll-combat-dice-p39c` | `supabase/functions/game-roll-combat-dice/index.ts` |
| `fn-game-roll-ground-combat-dice-p39c` | `supabase/functions/game-roll-ground-combat-dice/index.ts` |
| `fn-game-fire-bombardment-p39c` | `supabase/functions/game-fire-bombardment/index.ts` |
| `fn-game-fire-space-cannon-p39c` | `supabase/functions/game-fire-space-cannon/index.ts` |
| `fn-game-fire-anti-fighter-barrage-p39c` | `supabase/functions/game-fire-anti-fighter-barrage/index.ts` |
| `fn-game-activate-system-p39c` | `supabase/functions/game-activate-system/index.ts` |
| `fn-game-move-ships-p39c` | `supabase/functions/game-move-ships/index.ts` |
| `fn-game-play-strategy-card-p39c` | `supabase/functions/game-play-strategy-card/index.ts` |
| `fn-game-cast-votes-p39c` | `supabase/functions/game-cast-votes/index.ts` |
| `fn-game-resolve-commander-reroll` | `supabase/functions/game-resolve-commander-reroll/index.ts` |
| `client-edgeFunctions-p39c` | `src/lib/edgeFunctions.js` | Add `unlockCommander`, `resolveCommanderReroll` wrappers |
| `hook-useLeaders-p39c` | `src/hooks/useLeaders.js` | Add `unlockCommander` call, commander passive window handling |
| `component-CommanderRerollModal` | `src/components/game/CommanderRerollModal.jsx` | Jol-Nar dice selection UI |
| `component-GameScreen-p39c` | `src/components/game/GameScreen.jsx` | Handle `commander_passive` and `commander_reroll` windows |
