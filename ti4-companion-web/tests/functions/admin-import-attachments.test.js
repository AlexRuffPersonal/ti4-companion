...
const makeRequest = (body) => _makeRequest('admin-import-attachments', body)
...
beforeAll(async () => {
  ...
  await import('../../../supabase/functions/admin-import-attachments/index.ts')
})
...
describe('admin-import-attachments', () => {
  ...
  it('returns 400 when name is not a string', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const res = await handler(makeRequest({ records: [{ name: null }] }))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/name/)
  })
  ...
  it('returns 200 with imported count on valid payload', async () => {
    requireServiceRole.mockResolvedValue('user-id')
    const records = [{ name: 'Terraform' }, { name: 'Nano-Forge' }]
    const res = await handler(makeRequest({ records }))
